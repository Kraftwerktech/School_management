      ...
      <div className="grid grid-cols-5  mt-4 mb-3 lg:grid-cols-6 gap-4">
        {exams.map((exam, index) => (
          <div key={index} className="text-center hover:bg-slate-200 bg-[#F4F4EA] p-4 rounded-lg">
            <p className=' font-semibold text-[18px] text-[#465049]'>{exam.class}</p>
            <p className=' text-[16px]  font-medium text-[#465049]'>{exam.time}</p>
            <p className='text-[16px] font-medium text-[#465049]'>Room: {exam.room}</p>
          </div>
        ))}
      </div>
    </div>
